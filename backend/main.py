...
async def chat_endpoint(request: ChatRequest, current_user: models.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
    ...
    rag_docs = rag_engine.query_memory(request.message, n_results=15, user_id=current_user.id)
    rag_context_list = []
    for doc in rag_docs:
    ...
    # 2. Retrieve Conversation History from DB (Per User) — last 20 messages
    history_records = db.query(models.ChatMessage).filter(
        models.ChatMessage.user_id == current_user.id,
        models.ChatMessage.role != "summary"
    ...
    full_context = f"[Current Time: {current_time_str}]\n[ข้อมูลผู้ใช้งาน]: ชื่อเล่นในระบบคือ \"{current_user.nickname}\"\n{rag_text}\n{history_text}"
    ...
    # --- Gemini File API ถูกลบออกแล้ว ---
    # ไฟล์ทุกประเภทถูก extract เป็น text และเก็บใน FAISS ตอน upload แล้ว
    # RAG จะดึงเนื้อหาที่เกี่ยวข้องมาให้ AI โดยอัตโนมัติ
    ...
    # 6. Auto-summarize every 20 messages → เก็บลง RAG เพื่อจำระยะยาว
    msg_count = db.query(models.ChatMessage).filter(
        models.ChatMessage.user_id == current_user.id,
        models.ChatMessage.role != "summary"
    ).count()
    if msg_count % 20 == 0 and msg_count > 0:
        try:
            ...
            recent.reverse()
            convo_text = "\n".join([
                f"{'User' if r.role == 'user' else 'Mali'}: {r.content}"
                for r in recent if r.role in ("user", "ai")
            ])
            summary_prompt = f"สรุปบทสนทนานี้ให้กระชับ เน้นข้อมูลสำคัญที่ควรจำ:\n\n{convo_text}"
            summary = await run_in_threadpool(llm.generate_reply, summary_prompt, "", "")
            if summary:
                title = f"Summary_{current_user.id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M')}"
                train_text_internal(title, f"[บทสนทนาสรุป] {summary}", user_id=current_user.id)
                print(f"📝 Auto-summary saved for user {current_user.id}")
        except Exception as e:
            print(f"Auto-summary failed (non-critical): {e}")
...
@app.post("/forget")
async def forget_endpoint(request: ForgetRequest, current_user: models.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
    ...
    # ลบจาก Gemini File API ด้วย (ถ้ามี)
    db_file = db.query(models.UploadedFile).filter(
        models.UploadedFile.filename == request.filename
    ).first()
    if db_file:
        gemini_file_service.delete_file_from_gemini(db_file.gemini_file_name)
        db.delete(db_file)
        db.commit()
    ...
    try:
        rag_engine.rebuild_user_index(user_id=owner_id, data_store_root=os.path.abspath(DATA_STORE_DIR))
    except Exception as e:
        print(f"Warning: RAG rebuild failed (non-critical): {e}")
...
async def train_endpoint(
    ...
    """Upload file — ถ้า Gemini API พร้อมจะใช้ Gemini File API (Deep Read) อัตโนมัติ"""
    ...
    target_user_id = None if scope == "global" else current_user.id

    # บันทึกไฟล์ลง disk ก่อนเสมอ
    ...
    gemini_uri = None
    gemini_name = None
    use_gemini = gemini_file_service.is_gemini_available() and gemini_file_service.is_supported_file(safe_filename)

    if use_gemini:
        # --- Gemini Deep Read: อ่านไฟล์ทั้งหมดแล้วเก็บลง RAG ครั้งเดียว ---
        try:
            extracted_text = await run_in_threadpool(
                gemini_file_service.extract_content_from_file,
                file_path,
                safe_filename,
            )
            # เก็บลง FAISS เหมือน RAG ปกติ
            current_date = datetime.datetime.now().strftime("%Y-%m-%d")
            rag_engine.add_documents(
                [extracted_text],
                metadatas=[{"source": safe_filename, "memory_date": current_date, "deep_read": True}],
                user_id=target_user_id,
            )
            status_msg = "Deep Read (Gemini → RAG)"
        except Exception as e:
            print(f"Gemini extraction failed, falling back to plain RAG: {e}")
            use_gemini = False

    if not use_gemini:
        # --- RAG fallback (text only) ---
        try:
            text = raw_content.decode("utf-8")
        except UnicodeDecodeError:
            text = raw_content.decode("utf-8", errors="ignore")
        rag_engine.add_documents([text], metadatas=[{"source": safe_filename}], user_id=target_user_id)
        status_msg = "RAG Memory"

    entry = {
        "filename": safe_filename,
        "original_filename": file.filename,
        "timestamp": datetime.datetime.now().isoformat(),
        "status": f"Success ({status_msg})",
        "user_id": target_user_id,
        "scope": "Global" if target_user_id is None else "Private",
        "deep_read": use_gemini,
    }
    save_history(entry)

    return {"filename": safe_filename, "status": status_msg, "deep_read": use_gemini}

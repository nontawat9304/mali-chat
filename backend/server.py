...
# HARDCODED ABSOLUTE PATH to ensure no CWD confusion
...
print("="*60)
print(f"🚀 AI BACKEND STARTED - CODE VERSION: THAI_FIX_V3_DEBUG_ACTIVE")
print(f"📂 CWD: {os.getcwd()}")
print(f"📂 DATA_STORE: {os.path.abspath(DATA_STORE_DIR)}")
print("="*60)
...
with open("startup.log", "w") as f:
    f.write(f"Server init started...\n")
    f.write(f"CWD: {os.getcwd()}\n")
    f.write(f"DATA_STORE_DIR: {os.path.abspath(DATA_STORE_DIR)}\n")
...
def train_text_internal(title: str, text: str, user_id: int = None):
    ...
    # Allow Thai characters and spaces, block only invalid path chars
...
@app.get("/")
async def root():
    return {"status": "SERVER_ALIVE", "version": "V3.1"}
...
async def chat_endpoint(request: ChatRequest, current_user: models.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
    ...
    print(f"[DEBUG] RAG Context Sent to LLM:\n{rag_text}\n{'='*20}")
    ...
    print(f"[DEBUG] History loaded: {len(history_records)} items. Length: {len(history_text)}")
...
async def forget_endpoint(request: ForgetRequest, current_user: models.User = Depends(auth.get_current_user)):
    ...
    # FIX: Use correct function name 'rebuild_user_index' and pass user_id
    # If scope is Global, user_id is None. If Private, user_id is owner_id.
    target_uid = owner_id if not is_global else None
    print(f"[FORGET] Rebuilding index for User ID: {target_uid}...")
    rag_engine.rebuild_user_index(user_id=target_uid, data_store_root=DATA_STORE_DIR)
...
async def download_file(filename: str):
    ...
    global_path = os.path.join(DATA_STORE_DIR, "global", filename)
    print(f"[DEBUG] Download - Checking Global: {global_path}")
    if os.path.exists(global_path):
        return FileResponse(global_path, filename=filename)
    
    # Try private store (iterate all users? No, assume user context is not passed here for simplicity OR we need logic)
    # The current download button uses a simple href link which doesn't pass Auth Headers easily.
    # For now, let's just search recursively in data_store? 
    # Or better: search in ALL subdirectories of DATA_STORE_DIR
    
    for root, dirs, files in os.walk(DATA_STORE_DIR):
        # Normalize files on disk for comparison
        ...
        # print(f"[DEBUG] Scanning {root}: {list(files_normalized.keys())}")
        
        if filename_nfc in files_normalized:
            real_filename = files_normalized[filename_nfc]
            found_path = os.path.join(root, real_filename)
            print(f"[DEBUG] Download - Found at: {found_path}")
            return FileResponse(found_path, filename=real_filename)
            
    print(f"[DEBUG] Download - Failed to find: {filename} (NFC: {filename_nfc})")
    # PRINT ALL FILES to help debug
    print(f"[DEBUG] Only found these files:")
    for root, dirs, files in os.walk(DATA_STORE_DIR):
        for f in files:
            print(f" - {f} (NFC: {unicodedata.normalize('NFC', f)})")

    raise HTTPException(status_code=404, detail=f"File not found: {filename}")

@app.get("/train/content/{filename}")
async def get_file_content(filename: str):
    print(f"[DEBUG] GetContent - Requested: {filename}")
    # Normalize input
    filename_nfc = unicodedata.normalize('NFC', filename)
    print(f"[DEBUG] GetContent - Raw Hex: {filename.encode('utf-8').hex()}")
    print(f"[DEBUG] GetContent - NFC Hex: {filename_nfc.encode('utf-8').hex()}")
    
    # Search for file
    scanned = []
    for root, dirs, files in os.walk(DATA_STORE_DIR):
        files_normalized = {unicodedata.normalize('NFC', f): f for f in files}
        scanned.extend(list(files_normalized.keys()))
        
        if filename_nfc in files_normalized:
            real_filename = files_normalized[filename_nfc]
            try:
                with open(os.path.join(root, real_filename), "r", encoding="utf-8") as f:
                    return {"content": f.read()}
            except Exception as e:
                print(f"READ CONTENT ERROR ({filename}): {e}")
                raise HTTPException(status_code=400, detail=f"Cannot read file: {str(e)}")
    
    debug_msg = f"File not found: {filename} (NFC: {filename_nfc}). Scanned {len(scanned)} files."
    import json
    with open("scan_debug.log", "w", encoding="utf-8") as f:
         json.dump({"target": filename, "nfc": filename_nfc, "scanned": scanned}, f, ensure_ascii=False, indent=2)
    print(debug_msg)
    raise HTTPException(status_code=404, detail=debug_msg)

@app.get("/train/debug/list")
async def debug_list_files():
    scanned = []
    for root, dirs, files in os.walk(DATA_STORE_DIR):
        for f in files:
            scanned.append(os.path.join(root, f))
    return {"files": scanned, "data_store": DATA_STORE_DIR, "cwd": os.getcwd()}

@app.get("/train/content_v2")
async def get_file_content_query(filename: str):
    # FALLBACK: Query Parameter Support (Avoids Path Encoding Issues)
...
@app.post("/train/edit")
async def edit_file_content(request: EditRequest):
    # Normalize input filename
    request_filename_nfc = unicodedata.normalize('NFC', request.filename)

    # Search for file to overwrite
    for root, dirs, files in os.walk(DATA_STORE_DIR):
        # Normalize files on disk
        files_normalized = {unicodedata.normalize('NFC', f): f for f in files}

        if request_filename_nfc in files_normalized:
             real_filename = files_normalized[request_filename_nfc]
             file_path = os.path.join(root, real_filename)
             with open(file_path, "w", encoding="utf-8") as f:
                 f.write(request.content)
             
             # FIX: Update RAG immediately
             print(f"[RAG] Updating memory for {real_filename}...")
             
             # Determine User ID from path (naive but effective for now)
             # Path usually: data_store/users/{user_id}/{filename}
             target_user_id = None
             if "users" in root:
                 try:
                     # root like ...\data_store\users\1
                     parts = root.split(os.sep)
                     if "users" in parts:
                         idx = parts.index("users")
                         if idx + 1 < len(parts):
                             target_user_id = int(parts[idx+1])
                 except:
                     pass
             
             rag_engine.rebuild_user_index(user_id=target_user_id, data_store_root=DATA_STORE_DIR)
             
             return {"status": "File updated & Validated", "filename": real_filename}

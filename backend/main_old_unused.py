...
# DEBUG LOGGER
with open("startup_main.log", "w") as f:
    f.write("Main Server init started...\n")
...
async def get_file_content(filename: str):
    ...
    debug_msg = f"File not found: {filename} (NFC: {filename_nfc}). Scanned {len(scanned)} files: {str(scanned)[:200]}..."
    print(debug_msg)
...
async def edit_file_content(request: EditRequest):
    ...
    for root, dirs, files in os.walk(DATA_STORE_DIR):
        ...
        if request_filename_nfc in files_normalized:
             ...
             return {"status": "File updated", "filename": real_filename}
